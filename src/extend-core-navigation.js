 ...
 */
const withPriorityNavControls = createHigherOrderComponent(
	( BlockEdit ) => {
		return ( props ) => {
			...
			const { priorityNavEnabled, priorityNavMoreLabel, priorityNavMoreIcon } = attributes;
					...
					<InspectorControls>
						<PanelBody title={ __( 'Priority+ Settings', 'priority-nav' ) }>
							<TextControl
								...
								onChange={ ( value ) => setAttributes( { priorityNavMoreLabel: value } ) }
							...
							<SelectControl
								...
								options={ [
									{ label: __( 'Horizontal Dots (•••)', 'priority-nav' ), value: 'dots' },
									{ label: __( 'Chevron Down (▼)', 'priority-nav' ), value: 'chevron' },
									{ label: __( 'Plus (+)', 'priority-nav' ), value: 'plus' },
									{ label: __( 'Menu (≡)', 'priority-nav' ), value: 'menu' }
								] }
								onChange={ ( value ) => setAttributes( { priorityNavMoreIcon: value } ) }
							/>
						</PanelBody>
					</InspectorControls>
				</Fragment>
